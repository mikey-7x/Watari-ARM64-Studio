#!/bin/bash
# ==============================================================================
# Script: Watari Native Forge PRO v3.1 - Universal Master Installer
...
#              Android app compilation on Linux ARM64 (Termux & All Linux Distros).
# Smart Features: Pre-flight network check, Standalone Portable Gradle Engine.
# ==============================================================================
set -e
...
echo -e "${BLUE}======================================================${RESET}"
echo -e "${CYAN}   INITIALIZING WATARI PRO FORGE (KOTLIN/GRADLE) v3.1 ${RESET}"
echo -e "${BLUE}======================================================${RESET}"

# --- SMART FEATURE 1: Pre-flight Internet Check ---
...
# 2. Universal Dependency Installation (Removed Gradle to avoid repo errors)
echo -e "${YELLOW}[*] Step 1: Installing System Dependencies (Java 17 & Core Tools)...${RESET}"
...
if [ -n "$PREFIX" ] && [ -x "$PREFIX/bin/pkg" ]; then
    ...
    # Handle sudo dynamically for Linux Distros
    SUDO=""
    if [ "$(id -u)" -ne 0 ]; then
        if command -v sudo >/dev/null 2>&1; then
            SUDO="sudo"
        else
            echo -e "${RED}[!] WARNING: Not running as root and 'sudo' is missing.${RESET}"
        fi
    fi

    # Detect Package Manager
    if command -v apt-get >/dev/null 2>&1; then
        echo -e "${CYAN}[*] APT Package Manager Detected (Debian/Ubuntu)${RESET}"
        $SUDO apt-get update -y
        $SUDO apt-get install -y openjdk-17-jdk wget curl unzip zip git tree
    elif command -v pacman >/dev/null 2>&1; then
        echo -e "${CYAN}[*] Pacman Package Manager Detected (Arch Linux)${RESET}"
        $SUDO pacman -Sy --noconfirm jre17-openjdk jdk17-openjdk wget curl unzip zip git tree
    elif command -v xbps-install >/dev/null 2>&1; then
        echo -e "${CYAN}[*] XBPS Package Manager Detected (Void Linux)${RESET}"
        $SUDO xbps-install -Sy openjdk17 wget curl unzip zip git tree
    elif command -v dnf >/dev/null 2>&1; then
        echo -e "${CYAN}[*] DNF Package Manager Detected (Fedora/RHEL)${RESET}"
        $SUDO dnf install -y java-17-openjdk wget curl unzip zip git tree
    elif command -v zypper >/dev/null 2>&1; then
        echo -e "${CYAN}[*] Zypper Package Manager Detected (openSUSE)${RESET}"
        $SUDO zypper install -y java-17-openjdk wget curl unzip zip git tree
    else
        echo -e "${RED}[!] Unsupported package manager. Please install Java 17, wget, unzip, git manually.${RESET}"
    fi
fi

# Verify Java is actually installed
if ! command -v java >/dev/null 2>&1; then
    echo -e "${RED}[!] Fatal Error: Java 17 is not installed or not in PATH. Setup cannot continue.${RESET}"
    exit 1
fi
...
echo -e "${YELLOW}[*] Step 2: Generating Watari Workspace Directory...${RESET}"
rm -rf "$WATARI_HOME"
mkdir -p "$ANDROID_HOME/cmdline-tools"
mkdir -p "$WATARI_BIN"
mkdir -p "$WATARI_HOME/gradle"

# --- SMART FEATURE 2: Standalone Portable Gradle Engine ---
echo -e "${YELLOW}[*] Step 3: Fetching Portable Standalone Gradle Engine...${RESET}"
GRADLE_URL="https://services.gradle.org/distributions/gradle-8.7-bin.zip"
wget --show-progress -q -O "$WATARI_HOME/gradle/gradle.zip" "$GRADLE_URL"
...
# Inject into current session so watari-init works during testing
export PATH="$GRADLE_HOME/bin:$PATH"

# 4. Download SDK Command Line Tools
SDK_URL="https://dl.google.com/android/repository/commandlinetools-linux-11076708_latest.zip"
echo -e "${YELLOW}[*] Step 4: Fetching Official Android SDK CLI Tools...${RESET}"
wget --show-progress -q -O "$WATARI_HOME/cmdline-tools.zip" "$SDK_URL"
...
echo -e "${YELLOW}[*] Step 5: Installing Android Platforms & Build Tools...${RESET}"
set +e 
...
# 8. Inject Path Safely
echo -e "${YELLOW}[*] Step 7: Injecting Environment Variables...${RESET}"
for rc_file in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc_file" ] || [ "$rc_file" == "$HOME/.bashrc" ]; then
        if ! grep -q "watari_forge/bin" "$rc_file"; then
            ...
            # Injects the new standalone Gradle PATH alongside the others
